...
#pragma acc data copyout(vx[:nz][:nx], vz[:nz][:nx], uz[:nz][:nx], ux[:nz][:nx], We[:nz][:nx], s1,s2,s3,s4,s5)
    {
    ...
    std::cout<<"\nBB update_v2 s1= "<<s1<<"\n";
    std::cout<<"B update_v2 s2= "<<s2<<"\n";
    std::cout<<"B update_v2 s3= "<<s3<<"\n";
    std::cout<<"B update_v2 s4= "<<s4<<"\n";
    std::cout<<"B update_v2 s5= "<<s5<<"\n";



#pragma acc data present(vx[:nz][:nx], vz[:nz][:nx], uz[:nz][:nx], ux[:nz][:nx], szz[:nz][:nx], szx[:nz][:nx], sxx[:nz][:nx], We[:nz][:nx])
    {
...
#pragma acc exit data  copyout(vx[:nz][:nx], vz[:nz][:nx], uz[:nz][:nx], ux[:nz][:nx], We[:nz][:nx])
    ...
    #pragma acc enter data  copyin(vx[:nz][:nx], vz[:nz][:nx], uz[:nz][:nx], ux[:nz][:nx], We[:nz][:nx])
    std::cout<<"\nR update_v2 s1= "<<s1<<"\n";
    std::cout<<"R update_v2 s2= "<<s2<<"\n";
    std::cout<<"R update_v2 s3= "<<s3<<"\n";
    std::cout<<"R update_v2 s4= "<<s4<<"\n";
    std::cout<<"R update_v2 s5= "<<s5<<"\n";
...
#pragma acc data create(mem_vz_z[:nz][:nx], mem_vx_z[:nz][:nx], mem_vz_x[:nz][:nx], mem_vx_x[:nz][:nx])
    {
#pragma acc parallel loop independent
        for (int iz = 0; iz < nz; iz++)
        {
...
#pragma acc data create(grad_lam[:nz][:nx], grad_mu[:nz][:nx], grad_rho[:nz][:nx])
    {
...
#pragma acc data present(vz_z[:nz][:nx], vx_z[:nz][:nx], vz_x[:nz][:nx], vx_x[:nz][:nx], vz[:nz][:nx], vx[:nz][:nx])
    {
        #pragma acc parallel loop independent
...
#pragma acc exit data copyout(vz_z[:nz][:nx], vx_z[:nz][:nx], vz_x[:nz][:nx], vx_x[:nz][:nx], vz[:nz][:nx], vx[:nz][:nx])
...
#pragma acc enter data copyin(vz_z[:nz][:nx], vx_z[:nz][:nx], vz_x[:nz][:nx], vx_x[:nz][:nx], vz[:nz][:nx], vx[:nz][:nx])

    std::cout << "vDiff2= v1= " << v1 << "\n";
    std::cout << "vDiff2= v2= " << v2 << "\n";

    std::cout << "vDiff2= v3= " << v3 << "\n";
    std::cout << "vDiff2= v4= " << v4 << "\n";
}
...
#pragma acc data present(mem_z_z[:nz][:nx], mem_x_z[:nz][:nx], dz_z[:nz][:nx], dx_z[:nz][:nx], b_z[:nz], a_z[:nz], b_half_z[:nz], a_half_z[:nz], K_z[:nz], K_half_z[:nz])
#pragma acc data present(dx_x[:nz][:nx], dz_x[:nz][:nx], mem_x_x[:nz][:nx], mem_z_x[:nz][:nx])
#pragma acc data present(b_x[:nx], a_x[:nx], b_half_x[:nx], a_half_x[:nx], K_x[:nx], K_half_x[:nx])
    {
#pragma acc parallel loop default(none) collapse(2)
        for (int iz = nz1; iz < nz2; iz++)
        {
            for (int ix = nx1; ix < nx2; ix++)
            {
                if (pml_z)
                {
                    ...
                    mem_z_z[iz][ix] = b_z[iz] * mem_z_z[iz][ix] + a_z[iz] * dz_z[iz][ix];
                    mem_x_z[iz][ix] = b_half_z[iz] * mem_x_z[iz][ix] + a_half_z[iz] * dx_z[iz][ix];
                ...
                if (pml_x)
                {
                    ...
                    mem_x_x[iz][ix] = b_x[ix] * mem_x_x[iz][ix] + a_x[ix] * dx_x[iz][ix];
                    mem_z_x[iz][ix] = b_half_x[ix] * mem_z_x[iz][ix] + a_half_x[ix] * dz_x[iz][ix];
...
#pragma acc data present(szz[:nz][:nx], szx[:nz][:nx], sxx[:nz][:nx], mu_zx[:nz][:nx], lam[:nz][:nx], mu[:nz][:nx], vx_x[:nz][:nx], vz_z[:nz][:nx], vz_x[:nz][:nx])
#pragma acc data present(vx_z[:nz][:nx])
    {
...
#pragma acc loop independent
            for (int ix = nx1; ix < nx2; ix++)
            {
                ...
                // updating stresses
                szz[iz][ix] += dt * (lam[iz][ix] * (vx_x[iz][ix] + vz_z[iz][ix]) + (2.0 * mu[iz][ix] * vz_z[iz][ix]));
                szx[iz][ix] += dt * mu_zx[iz][ix] * (vz_x[iz][ix] + vx_z[iz][ix]);
                sxx[iz][ix] += dt * (lam[iz][ix] * (vx_x[iz][ix] + vz_z[iz][ix]) + (2.0 * mu[iz][ix] * vx_x[iz][ix]));
            }
        }
    }
      ...
      /// TEST  /?????????
double v1 = 0.0, v2 = 0.0, v3 = 0.0, v4 = 0.0;
#pragma acc data copyout(szz[:nz][:nx], szx[:nz][:nx], sxx[:nz][:nx])
{
    ...
    std::cout << "update_s2= v1= " << v1 << "\n";
    std::cout << "update_s2= v2= " << v2 << "\n";
    std::cout << "update_s2= v3= " << v3 << "\n";
    ...
    real *hc, int nz, int nx)
{
    ...
    real dxi = 1.0 / dx;
    real dzi = 1.0 / dz; // inverse of dx and dzlam
#pragma acc data present(szz[:nz][:nx], szx[:nz][:nx], sxx[:nz][:nx], szz_z[:nz][:nx], szz_z[:nz][:nx], szx_z[:nz][:nx], szx_x[:nz][:nx], sxx_x[:nz][:nx])
    {
        // 2D space grid
    ...
    int nz1, int nz2, int nx1, int nx2, real dt, int nz, int nx)
{
 ...
 double s1=0,s2=0,s3=0,s4=0, s5=0;
#pragma acc data copyout(vx[:nz][:nx], vz[:nz][:nx], uz[:nz][:nx], ux[:nz][:nx], We[:nz][:nx])
    {
...
#pragma acc data present(szz_z[:nz][:nx], szx_z[:nz][:nx], szx_x[:nz][:nx], sxx_x[:nz][:nx], rho_zp[:nz][:nx], rho_xp[:nz][:nx])
#pragma acc data present(uz[:nz][:nx], ux[:nz][:nx], vz[:nz][:nx], vx[:nz][:nx], We[:nz][:nx])
    {
...
#pragma acc data copyout(vx[:nz][:nx], vz[:nz][:nx], uz[:nz][:nx], ux[:nz][:nx], We[:nz][:nx])
    ...
    std::cout<<"\nupdate_v2 s1= "<<s1<<"\n";
    std::cout<<"update_v2 s2= "<<s2<<"\n";
    std::cout<<"update_v2 s3= "<<s3<<"\n";
    std::cout<<"update_v2 s4= "<<s4<<"\n";
    std::cout<<"update_v2 s5= "<<s5<<"\n";
...
#pragma acc data present(szz[:nz][:nx], szx[:nz][:nx], sxx[:nz][:nx], mu[:nz][:nx], lam[:nz][:nx], vx_x[:nz][:nx], vz_z[:nz][:nx])
    {
        int isurf;
        // -----------------------------
        // 1. TOP SURFACE
...
#pragma acc parallel loop independent
            for (int ix = nx1; ix < nx2; ix++)
            {
                ...
                szz[isurf][ix] = 0.0;
                szx[isurf][ix] = 0.0;
                sxx[isurf][ix] = 4.0 * dt * vx_x[isurf][ix] * (lam[isurf][ix] * mu[isurf][ix] + mu[isurf][ix] * mu[isurf][ix]) / (lam[isurf][ix] + 2.0 * mu[isurf][ix]);
#pragma acc loop independent
                for (int sz = 1; sz < isurf - nz1 + 1; sz++)
                { // mirroring
...
#pragma acc parallel loop independent
            for (int iz = nz1; iz < nz2; iz++)
            {
                ...
                sxx[iz][isurf] = 0.0;
                szx[iz][isurf] = 0.0;
                szz[iz][isurf] = 4.0 * dt * vz_z[iz][isurf] * (lam[iz][isurf] * mu[iz][isurf] + mu[iz][isurf] * mu[iz][isurf]) / (lam[iz][isurf] + 2.0 * mu[iz][isurf]);

#pragma acc loop independent
                for (int sx = 1; sx < isurf - nx1 + 1; sx++)
                { // mirroring
...
#pragma acc data present(accu_vz[:snap_nt][:snap_nz][:snap_nx], accu_vx[:snap_nt][:snap_nz][:snap_nx], accu_szz[:snap_nt][:snap_nz][:snap_nx], accu_szx[:snap_nt][:snap_nz][:snap_nx], accu_sxx[:snap_nt][:snap_nz][:snap_nx])
#pragma acc data present(szz[:nz][:nx], szx[:nz][:nx], sxx[:nz][:nx], vx[:nz][:nx], vz[:nz][:nx])
    {
...
#pragma acc  data copyout(accu_vz[:snap_nt][:snap_nz][:snap_nx], accu_vx[:snap_nt][:snap_nz][:snap_nx], accu_szz[:snap_nt][:snap_nz][:snap_nx], accu_szx[:snap_nt][:snap_nz][:snap_nx], accu_sxx[:snap_nt][:snap_nz][:snap_nx])
{
    ...
    int snap_x1, int snap_x2, int snap_dz, int snap_dx, int nz, int nx, int snap_nt, int snap_nz, int snap_nx)
{
    ...
    jz = 0;
#pragma acc data present(grad_rho[:nz][:nx], grad_mu[:nz][:nx], grad_lam[:nz][:nx])
#pragma acc data present(accu_vz[:snap_nt][:snap_nz][:snap_nx], szz[:nz][:nx], uz[:nz][:nx], ux[:nz][:nx], sxx[:nz][:nx], lam[:nz][:nx], szx[:nz][:nx], mu[:nz][:nx], accu_vx[:snap_nt][:snap_nz][:snap_nx], accu_szz[:snap_nt][:snap_nz][:snap_nx], accu_szz[:snap_nt][:snap_nz][:snap_nx], accu_szx[:snap_nt][:snap_nz][:snap_nx], accu_sxx[:snap_nt][:snap_nz][:snap_nx])
    {
...
#pragma acc loop independent
            for (int ix = snap_x1; ix <= snap_x2; ix += snap_dx)
            {
                ...
                s1 = 0.25 * (accu_szz[tf][jz][jx] + accu_sxx[tf][jz][jx]) * (szz[iz][ix] + sxx[iz][ix]) / ((lam[iz][ix] + mu[iz][ix]) * (lam[iz][ix] + mu[iz][ix]));

                s2 = 0.25 * (accu_szz[tf][jz][jx] - accu_sxx[tf][jz][jx]) * (szz[iz][ix] - sxx[iz][ix]) / (mu[iz][ix] * mu[iz][ix]);
...
#pragma acc data present(stf_z[:nsrc][it], z_src[:nsrc], x_src[:nsrc], rho_zp, vz,vx,  stf_x[:nsrc][it], stf_z[:nsrc][it], rho_xp , src_shot_to_fire[:nsrc],  )
   {
    switch (stf_type)
    {

    case (0): // Displacement stf 
              #pragma acc parallel default(none) loop independent
           ...
           int it, real dt, real dz, real dx, int nz, int nx, int nt)
{
    ...
    #pragma acc data present(vz[:nz][:nx], vx[:nz][:nx], rtf_uz[:nrec][:nt], rtf_ux[:nrec][:nt] ,  rtf_ux[:nrec][:nt], rx[:nrec], rz[:nrec])
  { 
    if (rtf_type == 0)
    {
...
#pragma acc data present(rtf_ux_true[:nshot][:nseis][:nt], a_stf_ux[:nseis][:nt], a_stf_uz[:nseis][:nt], rtf_ux_mod[:nseis][:nt], rtf_uz_true[:nshot][:nseis][:nt], rtf_uz_mod[:nseis][:nt])
    {
        if (rtf_type == 0)
        {
    ...
    int snap_dz, int snap_dx, int nz, int nx)
{
    ...
    // -----------------------------------
    jz = 0;
#pragma acc data present(grad[:nz][:nx], grad_shot[:snap_nz][:snap_nx])
    {
#pragma acc parallel loop independent default(none) private(jz, jx)
        for (int iz = snap_z1; iz <= snap_z2; iz += snap_dz)
        {
    ...
    int snap_z1, int snap_z2, int snap_x1, int snap_x2, int nz, int nx)
{
    ...
    //#pragma acc data present(We[:nz][:nx], We_adj[:nz][:nx])
    ...
    int snap_z1, int snap_z2, int snap_x1, int snap_x2, int nz, int nx)
{
    ...
    //#pragma acc data present(grad_shot[:nz][:nx],grad[:nz][:nx] )
    if (mat_av > 0)
    {
        //#pragma acc parallel loop independent
        for (int iz = snap_z1; iz < snap_z2; iz++)
...
#pragma acc data present(mat_old[:nz][:nx], grad_mat[:nz][:nx], grad_mat[:nz][:nx])
    {
...
#pragma acc data present(rho[:nz][:nx], lam[:nz][:nx], rho[:nz][:nx], rho_copy[:nz][:nx], lam_copy[:nz][:nx], rho_copy[:nz][:nx])
    {
    ...
    int nz, int nx)
{
    ...
    double C_mu = 0.0;
    double C_rho = 0.0;
#pragma acc data present(mu_zx[:nz][:nx], mu[:nz][:nx], rho_zp[:nz][:nx], rho_xp[:nz][:nx], rho[:nz][:nx], lam[:nz][:nx])
    {
#pragma acc parallel loop independent default(none) reduction(+ \
                                                              : C_lam, C_mu, C_rho)
...
#pragma acc loop independent
            for (int ix = 0; ix < nx - 1; ix++)
            {
                ...
                mu_zx[iz][ix] = 4.0 / ((1.0 / mu[iz][ix]) + (1.0 / mu[iz][ix + 1]) + (1.0 / mu[iz + 1][ix]) + (1.0 / mu[iz + 1][ix + 1]));
...
#pragma acc data present(mu[:nz][:nx], rho[:nz][:nx], lam[:nz][:nx])
    {
            ...
            int &taper_l1, int &taper_l2, int &taper_r1, int &taper_r2)
{
    ...
    // Horizontal taper
#pragma acc data present(A[:nz][:nx])
    {
#pragma acc parallel loop
        for (int iz = 0; iz < nz; iz++)
        {
