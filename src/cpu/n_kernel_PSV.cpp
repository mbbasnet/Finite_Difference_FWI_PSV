...
void kernel_PSV(int ishot, // shot index
    ...
    // Reset kernels
    reset_sv2(vz, vx, uz, ux, sxx, szx, szz, We, nz, nx);

    if (pml_z){ // Reset PML memory arrays if they exist 
        reset_PML_memory2(mem_vz_z, mem_vx_z, mem_szz_z, mem_szx_z, nz, nx);
        
    }
    if (pml_x){ // Reset PML memory arrays if they exist 
        reset_PML_memory2(mem_vz_x, mem_vx_x, mem_sxx_x, mem_szx_x, nz, nx);
    }
...
//        double time1,time2;
      //  double net=0.0;yy
       // time1= omp_get_wtime();
        ...
        if(pml_z||pml_x){
            pml_diff2(pml_z, pml_x, dz_z, dx_z, dz_x, dx_x, 
