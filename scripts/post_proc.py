...
if (fwinv):
    ...
    maxiter = 10
    #mat_dat0 = read_tensor("./bin/iter"+np.str(0)+"_mat.bin", np.float64, (3, ndim[1], ndim[2]))
    for ii in range(0,maxiter,1):
        # reading data from csv file
        mat_dat = read_tensor("./io/bin_fwi2/iter"+np.str(ii)+"_mat.bin", np.float64, (3, ndim[1], ndim[2]))
